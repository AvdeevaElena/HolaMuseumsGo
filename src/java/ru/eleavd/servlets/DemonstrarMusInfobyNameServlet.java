...
import ru.eleavd.egb.DemonstrarMusInfobyNameBean;
import javax.servlet.http.HttpSession;
import ru.eleavd.dto.DataTransferObject;
@WebServlet(name = "DemonstrarMusInfobyNameServlet", urlPatterns = {"/DemonstrarMusInfobyNameServlet"})
public class DemonstrarMusInfobyNameServlet extends HttpServlet {
private static final Logger logger = Logger.getLogger(DemonstrarMusEventsbyNameServlet.class);
    @EJB
    private DemonstrarMusInfobyNameBean museumBean;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (mus_string !="null" && mus_string.length() > 0 ) {
                museums = museumBean.SeachPorMuseumName(mus_string); 
                if (!museums.isEmpty()) {
                    dto.setListOfMuseumFullInfobyName(museums); 
                } else {
                    session.setAttribute("error", " НЕТ такого музея : " + mus_string);
                }
            } else {
