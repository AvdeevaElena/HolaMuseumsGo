...
@Stateless
public class DemonstrarMusEventbyTag1AND2AND3Bean {
@Resource(name = "dataSource")
    private DataSource dataSource;
    private static final Logger logger = Logger.getLogger(DemonstrarMusEventbyTagBean.class);
    public List<MuseumsDTO> SeachPor3AndTagsParametr(String parameter1, String parameter2, String parameter3) {    
        Connection conn = null;      
        PreparedStatement psta = null;        
        List<MuseumsDTO> respuesta = new ArrayList<MuseumsDTO>(); // !! ноль не нужен был
        try {  
            conn = dataSource.getConnection();
            psta = conn.prepareStatement(MuseumsDTO.getSELECT_Mus_Event_ny1And2And3Tags());        
